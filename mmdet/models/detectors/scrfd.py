    ...
    def simple_test(self, img, img_metas, rescale=False):
        ...
        if torch.onnx.is_in_onnx_export():
            ...
            if self.bbox_head.use_kps:
                for c in kps_pred:
                    print(c.shape)
                return torch.cat([torch.cat(cls_score, dim=1), torch.cat(bbox_pred, dim=1),
                                  torch.cat(kps_pred, dim=1)], dim=2)
            else:
                return torch.cat([torch.cat(cls_score, dim=1), torch.cat(bbox_pred, dim=1)], dim=2)
            #return outs
